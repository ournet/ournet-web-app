...
export default class IndexPage extends React.Component<IndexViewModel> {
    render() {
        const { lang, head, locales, links, latestEvents, country, latestQuotes, config, project } = this.props;
        ...
        head.elements.push(<link key='events-rss' rel="alternate" type="application/rss+xml" title={locales.events()} href={newsUrl + links.news.rss.stories({ ul: lang })}></link>);
        head.elements.push(<link key='imortant-rss' rel="alternate" type="application/rss+xml" title={locales.important_news()} href={newsUrl + links.news.rss.stories.important({ ul: lang })}></link>);
            ...
            <CommonLayout {...this.props}>
                ...
                <main>
                    <div className='o-layout'>
                        {latestEvents.slice(0, 4).map(item => <div key={item.id} className='o-layout__item u-1/4@tablet u-1/2@mobile'>{EventListItem({ lang, country, item, links, timezone: config.timezone, view: 'card', project })}</div>)}
                    </div>
                    <div className='o-layout'>
                        ...
                        {latestEvents.slice(4, 7).map(item => <div key={item.id} className='o-layout__item u-1/4@tablet u-1/2@mobile'>{EventListItem({ lang, country, item, links, timezone: config.timezone, view: 'card', project })}</div>)}
                    </div>
                    ...
                    <div className='o-layout'>
                        {latestEvents.slice(7).map(item => <div key={item.id} className='o-layout__item u-1/2@mobile u-1/4@tablet'>{EventListItem({ lang, country, item, links, timezone: config.timezone, view: 'card', project })}</div>)}
                    </div>
                </main>
            </CommonLayout>
        )
