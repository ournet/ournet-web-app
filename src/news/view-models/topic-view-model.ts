...
export class TopicViewModelBuilder<
  T extends TopicViewModel,
  I extends TopicViewModelInput
> extends NewsViewModelBuilder<T, I> {
  async build() {
    ...
    head.description = locales.news_topic_description_format({
      name: topic.name
    });
    ...
    this.apiClient
      .newsEventsLatestByTopic(
        ...
        { params: { lang, country, limit: 4, topicId: topic.id } }
      )
      .newsItemsLatestByTopic(
        "topicNews",
        { fields: NewsItemStringFields },
        { params: { lang, country, limit: 5, topicId: topic.id } }
      )
      .quotesLatestByTopic(
        "aboutQuotes",
        { fields: QuoteStringFields },
        { params: { country, lang, limit: 3, topicId: topic.id } }
      );
    ...
    if (topic.type === "PERSON") {
      this.apiClient.quotesLatestByAuthor(
        "byQuotes",
        { fields: QuoteStringFields },
        { params: { country, lang, limit: 3, authorId: topic.id } }
      );
    }
    return super.build();
  }
