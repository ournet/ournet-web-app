...
export default class EventPage extends React.Component<EventViewModel> {
  render() {
    ...
    for (let tag of event.topics) {
      head.elements.push(
        <meta key={`tag-${tag.id}`} property="article:tag" content={tag.name} />
      );
    }
          ...
          <main>
                  ...
                  <div className="c-event__body">
                    <div className="o-layout o-layout--small">
                      ...
                      <div className="o-layout__item u-5/6@tablet">
                        ...
                        <div className="c-event__text">{paragraphs}</div>
                        {Share({
                          lang,
                          url: head.canonical,
                          align: "right",
                        ...
                        <div className="u-clearfix">
                          {OutReadMoreLink({
                            url: event.source.host + event.source.path,
                            source: startWithUpperCase(event.source.sourceId),
                            links,
                            locales
                          })}
                        </div>
                        {eventQuotes && (
                          <div className="c-event_quotes">
                            {eventQuotes.map((item) =>
                              QuoteListItem({
                                item,
                                view: "card",
                                country,
                ...
                {similarEvents.length > 0 ? (
                  <div className="c-section">
                    ...
                    <div key="slayout" className="o-layout">
                      {similarEvents.slice(0, 2).map((item) => (
                        <div
                          key={item.id}
            ...
            <script
              dangerouslySetInnerHTML={{
                __html: `(function(){var img=new Image();img.src='${links.news.actions.viewStory(
                  event.id,
                  { ul: lang }
                )}';}());`
              }}
