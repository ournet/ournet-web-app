...
export class SitemapRegionIndexHandler extends WeatherBaseHandler {
  async handle(data: WeatherAppData): Promise<void> {
    ...
    const siteMap = sm.buildSitemapIndex({
      ...
      urls: result.regions.map((item) =>
        fullLink(
          links.weather.sitemap.regionPlaces(item.admin1Code, { ul: lang })
        )
      )
    });

    return this.send(siteMap.toString(), 200, {
      "content-type": "text/xml; charset=utf-8"
    });
  }
}

export interface SitemapMainPlacesModelInput extends OurnetViewModelInput {
  countryCode: string;
}

export class SitemapMainPlacesHandler extends WeatherBaseHandler<SitemapMainPlacesModelInput> {
  async handle(data: WeatherAppData): Promise<void> {
    const { country, project, countryCode } = this.input;
    const schema = getSchema(project, country);
    const host = getHost(project, country);
    ...
    api.placesMainPlaces(
      "places",
      { fields: "id name featureCode population" },
      { country: countryCode, limit: 100 }
    );
...
export class SitemapRegionPlacesHandler extends WeatherBaseHandler<SitemapRegionPlacesModelInput> {
  async handle(data: WeatherAppData): Promise<void> {
    ...
    api.placesPlacesByAdmin1Code(
      "places",
      { fields: "id name featureCode population" },
      { country, admin1Code, limit: 1000 }
    );
