...
import {
  ...
  Widget1ViewModelBuilder
} from "../view-models/widget1-view-model";
import { Widget1Frame } from "../views/widget/widget1-frame";
import { getHost } from "ournet.links";
import { isNullOrEmpty } from "../../helpers";
import { Dictionary } from "@ournet/domain";
import {
  Widget2ViewModelInput,
  Widget2ViewModelBuilder
} from "../view-models/widget2-view-model";
...
export class Widget1FrameHandler extends WeatherBaseHandler<Widget1ViewModelInput> {
  async handle(data: WeatherAppData) {
    const viewData = await new Widget1ViewModelBuilder(
      this.input,
      data
    ).build();
...
export class Widget1HtmlStriptHandler extends WeatherBaseHandler<Widget1ViewModelInput> {
  async handle(appData: WeatherAppData) {
    const model = await new Widget1ViewModelBuilder(
      this.input,
      appData
    ).build();
    ...
    if (scripttype === "iframe") {
      data.push(
        '<iframe src="//' +
          host +
          links.weather.widget.widgetFrame(query as Dictionary<string>) +
          '" scrolling="no" frameborder="0" style="border:none;overflow:hidden;height:' +
          height +
          "px;width:" +
          width +
      ...
      data.push(
        '<ins class="ournetweather" style="display:block;max-width:' +
          width +
          "px;height:" +
          height +
          'px" data-type="widget" data-cn="' +
          country +
          '" data-params="' +
          params.join(";") +
          '" data-h="' +
...
export class Widget2FrameHandler extends WeatherBaseHandler<Widget2ViewModelInput> {
  async handle(data: WeatherAppData) {
    const viewData = await new Widget2ViewModelBuilder(
      this.input,
      data
    ).build();
...
export class Widget2HtmlStriptHandler extends WeatherBaseHandler<Widget2ViewModelInput> {
  async handle(appData: WeatherAppData) {
    const model = await new Widget2ViewModelBuilder(
      this.input,
      appData
    ).build();
    ...
    if (scripttype === "iframe") {
      ...
      data.push(
        '<iframe src="//' + host,
        links.weather.widget2.widgetFrame(query) +
          '" scrolling="no" frameborder="0" style="border:none;overflow:hidden;height:' +
          iframeHeight +
          "px;width:" +
          query.w +
