...
export function ForecastBrowser({ places, days, today, locales }: ForecastBrowserProps) {
    ...
    return (
        <div className='c-forbro' data-ids={places.join(',')}>
            <ul className='c-forbro__tabs'>
                {dates.map((date, index) => (<li key={index} className={index === 0 ? 'c-forbro__tabs--selected' : ''} data-date={date.toISOString().substr(0, 10)}>{index === 0 ? locales.today() : date.format('dddd')}</li>))}
            </ul>
