...
export default class CommonLayout extends React.Component<WeatherViewModel> {
    render() {
        const { project, children, lang, country, mainPlaces, config, translate, links, latestNews } = this.props;
        return (
            <Layout {...this.props}>
                    ...
                    <div className="o-layout__item u-2/6@desktop">
                        ...
                        {latestNews && latestNews.length > 0 &&
                            <div className='c-section'>
                                {SectionHeader({ name: translate(WeatherLocaleNames.latest_news), link: getSchema(OurnetProjectName.NEWS, country) + '//' + getHost(OurnetProjectName.NEWS, country) + links.news.home({ ul: lang }) })}
                                <div className="o-layout o-layout--small">
                                    {latestNews.map(item => <div key={item.id} className='o-layout__item u-1/2@tablet u-1/1@desktop'>{EventListItem({ lang, country, project, links, timezone: config.timezone, item, view: 'card-bare' })}</div>)}
                                </div>
                            </div>}
                        {AdAside()}
