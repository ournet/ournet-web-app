...
export function PlacesList({ lang, places, links, ref }: PlacesListProps) {
  ...
  const items = places.map((place) => {
    ...
    if (place.featureClass === "A") {
      link = (
        <a href={links.weather.places.byAdm1(place.admin1Code, linkParams)}>
      ...
      link = (
        <a
          href={
            links.weather.place(place.id.toString(), linkParams) +
            (ref ? "#ref-" + ref : "")
          }
        >
          {getPlaceName(place, lang)}
      ...
      if (place.admin1) {
        adm1 = (
          <div>{WeatherPlaceHelper.shortAdm1Name(place.admin1, lang)}</div>
        );
      }
    }
    return (
  ...
  return <div className="o-layout o-layout--small c-places-list">{items}</div>;
}
