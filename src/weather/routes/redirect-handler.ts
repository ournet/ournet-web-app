...
export class Widget2RedirectRouter extends OurnetRouter {
  ...
  protected createHander(req: Request, res: Response) {
    ...
    const handler = new RedirectHandler({
      code: 301,
      location: links.weather.widget({ ul: lang }),
      req,
      res
    });
...
const ROUTE_PREFIX =
  "(prognoza|pogoda|vremea|forecast|tempo|pocasi|elorejelzes|mot)";
...
export class PrefixPlacesRedirectRouter extends OurnetRouter {
  ...
  protected createHander(req: Request, res: Response) {
    ...
    const handler = new RedirectHandler({
      code: 301,
      location: links.weather.places({ ul: lang }),
      req,
      res
    });
...
export class PrefixPlacesAdminRedirectRouter extends OurnetRouter<PrefixPlacesRedirectRouterData> {
  ...
  protected createHander(
    req: Request,
    res: Response,
    data: PrefixPlacesRedirectRouterData
  ) {
    const input = this.formatInput(req, res);
    const config = createAppConfig(input.project, input.country);
    const links = sitemap(config.languages[0]);
    ...
    const handler = new RedirectHandler({
      code: 301,
      location: links.weather.places.byAdm1(data.admin1Code, { ul: lang }),
      req,
      res
    });
...
export class PrefixPlaceRedirectRouter extends OurnetRouter<PrefixPlaceRedirectRouterData> {
  ...
  protected createHander(
    req: Request,
    res: Response,
    data: PrefixPlaceRedirectRouterData
  ) {
    const input = this.formatInput(req, res);
    const config = createAppConfig(input.project, input.country);
    const links = sitemap(config.languages[0]);
    ...
    const handler = new RedirectHandler({
      code: 301,
      location: links.weather.place(data.placeId, { ul: lang }),
      req,
      res
    });
