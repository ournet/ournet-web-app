...
export class PlacesDailyForecastViewModelBuilder extends WeatherViewModelBuilder<
  PlacesDailyForecastViewModel,
...
> {
  async build() {
    ...
    if (result.data && result.data.places) {
      ...
      this.apiClient.weatherDatePlacesForecast(
        "data",
...
export interface PlacesDailyForecastViewModelInput
  extends OurnetViewModelInput {
  ids: string[];
  date: string;
}
