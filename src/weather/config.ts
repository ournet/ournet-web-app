...
export const WEATHER_GLOBAL_CONFIG_LIST_IDS = uniq(
  getSupportedCountries()
    .map((country) =>
      createAppConfig<WeatherAppConfig>(OurnetProjectName.WEATHER, country)
    )
    .map((config) => config.lists || [])
    .reduce<string[]>(
      (ids, list) => ids.concat(list.map((item) => item.id)),
      []
    )
);
